...
class Admin extends HM_Controller
{
    ...
    private function getInfo($uuid = 0)
    {
        ...
        $logs = $this->Main_model->select('raspberry_records', ['kw', 'cost', 'createdAt'], ['raspberry_uuid' => $this->raspbery['uuid']], 7, 'DESC', 'createdAt');
        $records = $this->getStatistics($this->raspbery['uuid']);
        $lastRecord = $this->Main_model->select('raspberry_records', ['kw', 'cost'], ['raspberry_uuid' => $this->raspbery['uuid']], 1, 'DESC', 'createdAt', 1);
    ...
    private function getStatistics($raspUUID = 0)
    {
        ...
        $records = $this->Main_model->get_query(
            'select (select MAX(kw) from raspberry_records rs WHERE ' . $formatType1 . ' = ' . $formatType2 . ' and raspberry_uuid = "' . $raspUUID . '" GROUP BY ' . $formatType1 . ') as maxKw, createdAt from raspberry_records r where '
