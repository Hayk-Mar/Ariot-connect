...
class Statistics extends HM_Controller
{
    ...
    private function getStatistics($raspUUID = 0, $dateRange = [], $period = 2)
    {
        ...
        $records = $this->Main_model->get_query(
            'select id, (select MAX(kw) from raspberry_records rs WHERE ' . $formatType1 . ' = ' . $formatType2 . ' and raspberry_uuid = "' . $raspUUID . '" GROUP BY ' . $formatType1 . ') as maxKw, createdAt from raspberry_records r where '
